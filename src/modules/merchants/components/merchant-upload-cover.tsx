...
import { useUploadMerchantCoverMutation } from '../../../api/merchantApi';

export function MerchantUploadCover({
  merchant,
  onChange,
}: {
  ...
  const [uploadCoverMutation] = useUploadMerchantCoverMutation()
  const onDrop = useCallback(async (files: Array<File>) => {
    if (files[0]) {
      const result = await uploadCoverMutation({
        id: merchant.id,
        file: files[0],
      }).unwrap()
      onChange(result)
      ...
      <input {...getInputProps()} />
      {
        ...
        >
          ...
          <div className="grow md:px-[15px]">
            <div style={{ fontSize: 14, fontWeight: 600 }}>Add a cover image</div>
            <div className="mt-[5px]" style={{ fontSize: 12 }}>
              Accepted: JPG, PNG, SVG, or WEBP. Recommended size: 1024 x 512 pixels.
            </div>
          </div>
